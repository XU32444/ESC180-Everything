...
def is_sequence_complete(board,col,ystart,xstart,length,dy,dx):
    if xstart != 0 and ystart != 0:
        if board[ystart - 1][xstart - 1] == col:
            return False
    if board[ystart + dy*(lenth)][xstart + dy*length] == col:
        return False
    for i in range(len(length)):
        if board[ystart + i*dy][xstart + i*dx] != col:
            return False
    return True
