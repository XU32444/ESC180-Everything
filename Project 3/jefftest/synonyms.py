...
def build_semantic_descriptors_from_files(filenames):
    ...
    for i in range(len(filenames)):
        # if /n/ becomes a problem, replace latin1 with utf-8
        file = open(filenames[i], "r", encoding="latin1").read()

        # All punctuation for sentences becomes uniform "."
        f1 = file.replace("!", ".")
        f2 = f1.replace("?", ".")
...
def most_similar_word(word, choices, semantic_descriptors, similarity_fn):
    ...
    for i in range(len(choices) - 1, 0, -1):  # reverse indices
        choice = choices[i]
        if choice != ' ':  # Skip spaces
            cosi = similarity_fn(semantic_descriptors.get(
