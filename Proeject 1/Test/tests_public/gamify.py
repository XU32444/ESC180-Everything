def initialize():
    ...
    prev_running_time = 0
    global prev_textbook_time
    prev_textbook_time = 0
    global last_time_run
    last_time_run = False
    global last_time_textbook
    last_time_textbook = False
        ...
        if star_running:#if a star is given to running
            ...
            if tiredness:#user tired
                ...
                if running_clock <= 180:
                    ...
                    cur_health = cur_health + (180 - prev_running_time) * 3 + (running_clock - 180) * 1
                #the hedon algorithm:
                if prev_running_time > 10:
                    cur_hedons = cur_hedons - duration * 2
                else:
                    if prev_running_time + duration <= 10:
                        cur_hedons = cur_hedons + duration * 2
                    else:
                        if duration <= (10 - prev_running_time ):
                            cur_hedons = cur_hedons + duration * 2
                        else:
                            cur_hedons = cur_hedons + (10 - prev_running_time) * 2 - (duration -(10 - prev_running_time)) * 2
                #the mandatory
                tiredness = True
                cur_time = cur_time + duration
        prev_running_time = prev_running_time + duration
        prev_textbook_time = 0
        ...
        if star_textbooks:#if star is given to textbooks
            ...
            if tiredness:#user tired
                ...
                if duration <= 20:
                    ...
                    if prev_textbook_time > 20:
                        cur_hedons = cur_hedons - duration * 1
                    else:
                        if prev_textbook_time + duration <= 20:
                            cur_hedons = cur_hedons + duration * 1
                        else:
                            if duration <= (20 - prev_running_time ):
                                cur_hedons = cur_hedons + duration * 1
                            else:
                                cur_hedons = cur_hedons + (20 - prev_running_time) * 1 - (duration -(20 - prev_running_time)) * 1
                #the mandatory
                tiredness = True
                cur_time = cur_time + duration
        prev_textbook_time = prev_textbook_time + duration
        prev_running_time = 0
